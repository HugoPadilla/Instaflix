...
class PopularMoviesRemoteMediator (
    ...
    override suspend fun initialize(): InitializeAction {
        ...
        return if (System.currentTimeMillis() - (moviesDatabase.getRemoteKeysDao().getCreationTime(Constants.CATEGORY_POPULAR) ?: 0) < cacheTimeout) {
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, MovieEntity>): RemoteKeys? {
        ...
        return state.pages.lastOrNull {
            ...
            moviesDatabase.getRemoteKeysDao().getRemoteKeyByMovieIDFilterByCategory(movie.id, Constants.CATEGORY_POPULAR)
        }
    }
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, MovieEntity>): RemoteKeys? {
        ...
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { id ->
                moviesDatabase.getRemoteKeysDao().getRemoteKeyByMovieIDFilterByCategory(id, Constants.CATEGORY_POPULAR)
            }
        }
    }
        ...
        try {
            ...
            moviesDatabase.withTransaction {
                if (loadType == LoadType.REFRESH) {
                    moviesDatabase.getRemoteKeysDao().clearRemoteKeys(Constants.CATEGORY_POPULAR)
                    moviesDatabase.getMovieCategoryDao().clearAllMovies(Constants.CATEGORY_POPULAR)
                }
                val prevKey = if (page > 1) page - 1 else null
                val nextKey = if (endOfPaginationReached) null else page + 1
                val remoteKeys = movies.map {
                    RemoteKeys(movieID = it.id, categoryId = Constants.CATEGORY_POPULAR, prevKey = prevKey, currentPage = page, nextKey = nextKey)
                }
                val movieCategoryEntity = movies.map {
                    MovieCategoryEntity(movieId = it.id, categoryId = Constants.CATEGORY_POPULAR, page = page)
                }

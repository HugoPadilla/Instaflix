...
class PopularTvShowRemoteMediator (
    ...
    override suspend fun initialize(): InitializeAction {
        ...
        return if (System.currentTimeMillis() - (moviesDatabase.getTvShowRemoteKeysDao().getCreationTime(Constants.CATEGORY_POPULAR) ?: 0) < cacheTimeout) {
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, TvShowEntity>): TvShowRemoteKeys? {
        ...
        return state.pages.lastOrNull {
            ...
            moviesDatabase.getTvShowRemoteKeysDao().getRemoteKeyByMovieIDFilterByCategory(movie.id, Constants.CATEGORY_POPULAR)
        }
    }
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, TvShowEntity>): TvShowRemoteKeys? {
        ...
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { id ->
                moviesDatabase.getTvShowRemoteKeysDao().getRemoteKeyByMovieIDFilterByCategory(id, Constants.CATEGORY_POPULAR)
            }
        }
    }
    ...
    override suspend fun load(
        ...
        try {
            val apiResponse = tvShowApiService.getPopular(page = page)
            ...
            moviesDatabase.withTransaction {
                if (loadType == LoadType.REFRESH) {
                    moviesDatabase.getTvShowRemoteKeysDao().clearRemoteKeys(Constants.CATEGORY_POPULAR)
                    moviesDatabase.getTvShowCategoryDao().clearAllTvShow(Constants.CATEGORY_POPULAR)
                }
                val prevKey = if (page > 1) page - 1 else null
                val nextKey = if (endOfPaginationReached) null else page + 1
                val remoteKeys = movies.map {
                    TvShowRemoteKeys(tvShowId = it.id, categoryId = Constants.CATEGORY_POPULAR, prevKey = prevKey, currentPage = page, nextKey = nextKey)
                }
                val movieCategoryEntity = movies.map {
                    TvShowCategoryEntity(tvShowId = it.id, categoryId = Constants.CATEGORY_POPULAR, page = page)
                }
                ...
                moviesDatabase.getTvShowCategoryDao().insertAll(movieCategoryEntity)
            }
